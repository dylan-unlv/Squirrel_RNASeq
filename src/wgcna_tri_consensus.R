...
TOM[2,,] = kidney_data$TOM[which(colnames(kidney_data$mat) %in% tgenes),which(colnames(kidney_data$mat) %in% tgenes)]
TOM[3,,] = liver_data$TOM[which(colnames(liver_data$mat) %in% tgenes),which(colnames(liver_data$mat) %in% tgenes)]



nGenes <- length(tgenes)
setLabels <- c('heart','kidney','liver')
...
multiExpr[[2]] <- list(data=kidney_data$mat[,tgenes])
