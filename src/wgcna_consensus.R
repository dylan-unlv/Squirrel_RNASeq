...
setLabels <- c('heart','brain','kidney','liver')
...
multiExpr[[3]] <- list(data=kidney_data$mat[,tgenes])
multiExpr[[4]] <- list(data=liver_data$mat[,tgenes])
...
pdf(file='figs/wgcna_consensus_module_dendrogram.pds', width=9, height=6)
sizeGrWindow(9,6)
...
saveRDS(list(consMEs=consMEs, moduleColors=moduleColors, 
             ...
             consensusTOM=consensusTOM), file = 'data/wgcna/consensus_wgcna_objs.R')
cyt <- exportNetworkToCytoscape(consensusTOM,
                                edgeFile = 'data/wgcna/consensus.edges',
                                nodeFile = 'data/wgcna/consensus.nodes',
                                weighted = T,
                                threshold = 0.02,
                                nodeNames = tgenes,
